    ...
    <div class="card">
      <div class="card-body">
        <ul v-if="user">
          ...
          <li>Email信箱</li>
            <span v-if="!isEditing">{{ authStore.user.email }}</span> <input v-if="isEditing" v-model="authStore.user.email" />
          
         </div>
        </ul>     
            <a href="#" class="button-17" @click="toggleEdit">{{ isEditing ? '保存' : '修改' }}</a>
      </div>
    </div>
  </template>
